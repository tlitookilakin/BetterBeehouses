	...
	class UtilityPatch
	{
		...
		internal static bool preCheck(GameLocation location, Vector2 startTileLocation, int range, Func<Crop, bool> additional_check, ref Crop __result)
		{
			if (ModEntry.config.UseRandomFlower)
			{
				...
				return false;
			} else if (ModEntry.config.UseGiantCrops)
			{
				foreach (var clump in location.resourceClumps)
				{
					if (clump is GiantCrop giant && IndexIsFlower(giant.parentSheetIndex.Value))
					{
						for (int x = 0; x < giant.width.Value; x++)
						{
							...
							{
								if (Math.Abs(giant.tile.X + x - startTileLocation.X) + Math.Abs(giant.tile.Y + y - startTileLocation.Y) <= range)
								{
									__result = CropFromIndex(giant.parentSheetIndex.Value);
									return false;
								}
							}
						}
			...
			if(loc.objects.TryGetValue(tile, out StardewValley.Object obj))
			{
				...
				} else
				{
					return ModEntry.config.UseForageFlowers && obj.CanBeGrabbed && 
						ObjectIsFlower(obj) ? CropFromObj(obj) : null;
					//non-pot forage
				}
			} else if(ModEntry.config.UseFruitTrees && loc.terrainFeatures.TryGetValue(tile, out var tf) && tf is FruitTree tree)
			{
				if (tree.fruitsOnTree.Value > 0 && IndexIsFlower(tree.indexOfFruit.Value))
					return new Crop(tree.indexOfFruit.Value, (int)tile.X, (int)tile.Y);
			}
			return null;
		}
		public static bool IsGrown(Crop crop, Func<Crop, bool> extraCheck = null)
		{
		...
		public static Crop CropFromIndex(int index)
		{
			Crop ret = new();
